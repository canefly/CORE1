<?php 
// Standard database include from your FINANCE/includes folder
// Inayos natin ang path (nilagyan ng ../)
...
try {
    ...
    // Pinalitan natin ang $conn->query ng PDO statements
    $stmt = $pdo->prepare($query);
    $stmt->execute();
    
    // Ise-save natin lahat ng data dito sa $returned_apps array
    $returned_apps = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        <div class="content-card">
            <table>
                ...
                <tbody id="returnedTable">
                    <?php if (!empty($returned_apps)): ?>
                        <?php foreach($returned_apps as $row): ?>
                        <tr>
                            <td style="color:#ef4444; font-weight:700;">#LA-<?php echo htmlspecialchars($row['id']); ?></td>
                            <td>
                            ...
                            <td style="text-align:center;">
                                <a href="view_details.php?id=<?php echo urlencode($row['id']); ?>" class="btn-view" title="View Details">
                                    <i class="bi bi-eye"></i>
                                </a>
                            </td>
                        </tr>
